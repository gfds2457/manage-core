...
> 全栈后台管理系统：业务后端 + AI 后端双服务架构。覆盖 RBAC 权限、商品中心、上下架审核流，以及基于本地知识库 RAG 的流式 AI 问答。

## 项目介绍

- 基于 **Vue3 + TypeScript + Node.js（Express）+ MySQL + LanceDB 向量库 + OpenAI 兼容大模型** 实现的商品后台管理系统
- 提供商品管理、用户管理、角色管理、菜单管理、上下架审核、操作日志、数据大屏、AI 问答等功能
- 项目按三个阶段演进（架构决策可结合「项目亮点」阅读）：
  1. **前端阶段**：Vue3 单页应用，接口层基于 vite-plugin-mock
  2. **后端化阶段**：将 60 条 mock 路由整体迁移为独立 Express 服务（统一 adapter 注册、鉴权前置、统一 JSON 返回）
  3. **AI 阶段**：独立 AI 服务，落地 RAG 本地知识库、工具调用、SSE 流式对话与记忆系统
- 业务后端接口契约基于开源项目「硅谷甄选」重构扩展，补充了软删除回收站、审核流、操作日志等深层需求（下文统一称「硅谷甄选」）
...
- 商品增删改查、上下架
  ...
  - 上下架审核：提交审核单 / 审核分页列表 / 通过 / 驳回（驳回必须填写理由）/ 批量通过与驳回；审核与商品写权限分离（独立权限码 `goods:review:audit`）
...
- AI 问答（本地知识库 RAG）：
  ...
  - 工具调用：服务端注册本地工具（如天气查询）与前端卡片工具（手机品牌信息卡片），模型按语义触发，卡片参数经 SSE 下发渲染
  - Markdown 渲染
  - RAG 本地知识库：检索前 Query 改写 → Embedding → LanceDB 向量检索 → 相似度阈值过滤 → 按相关度降序排序并展示来源（可溯源）；检索异常自动降级为无检索模式；知识库支持业务侧增量增删改（带令牌鉴权）

## 技术栈

- 前端：Vue 3 + TypeScript + Vite + Element Plus + Pinia + ECharts；SSE 客户端使用 `@microsoft/fetch-event-source`
- 业务后端：Node.js + TypeScript（tsx 运行）+ Express 5 + CORS
- AI 后端：Node.js（ESM）+ Express 5 + MySQL（mysql2）+ LanceDB 向量库 + LangChain + OpenAI SDK + MCP SDK + multer
- 工程化：Vitest 单元测试与覆盖率、ESLint + Prettier + Stylelint、配置中心启动期校验（缺配置直接拒绝启动并给出修复提示）、`/health` 健康检查、concurrently 一键并发启动

## 项目亮点

1. 独立完成项目骨架搭建（Vue3 界面、后端接口与数据结构设计）；基于开源「硅谷甄选」后台项目重新拆解架构，并补充深层需求功能
...
7. 业务增强功能：面向多人协作场景，增加商品软删除与回收站恢复、上下架审核流（审核与商品写权限分离）、高敏感操作日志留痕

## 项目结构

```
manageCode/
├── admin-front/                # 前端项目（Vue3 + Vite + TypeScript）
│   ├── src/
│   │   ├── API/                # 接口层，按模块划分（user / role / menu / product / spu / sku / audit / ai-chat / knowledge / log / dashboard / category）
│   │   ├── assets/             # 静态资源
│   │   ├── components/         # 通用组件（登录页、404、分类级联等）
│   │   ├── composables/        # 组合式函数（useRecycleBin 回收站复用逻辑）
│   │   ├── directive/          # 自定义指令（v-hasBtn 按钮级权限）
│   │   ├── router/             # 路由
│   │   ├── store/              # Pinia 状态管理（user / route / category / quick-chat 等）
│   │   ├── setting/            # 站点配置（logo 等）
│   │   ├── styles/             # 全局样式
│   │   ├── utils/              # 工具（request、permission.ts 权限码定义、aiEnv / aiError 健康检查与降级提示）
│   │   └── views/              # 页面：home（首页布局）/ dashboard（首页图表与 AI 面板）/ goods（brand·attr·spu·sku·review）/ permission（user·role·menu）/ ai-chat / knowledge / data-screen（数据大屏）
│   └── .env.development        # VITE_API_BASE_URL（业务后端）/ VITE_AI_API_BASE_URL（AI 后端）
│
├── admin-mock-backend/         # 业务后端（Express 5 + TypeScript，内存态数据，60 条路由）
│   └── src/
│       ├── index.ts            # 入口：CORS + 路由注册 + 404 JSON 兜底
│       ├── adapter.ts          # 统一路由出口：鉴权前置 / 知识库同步 / 操作日志 / 统一错误 JSON
│       ├── mock/               # audit / brand / attribute / log / role / sku / spu / user 共 8 个业务模块
...
│
├── admin-ai-backend/           # AI 后端（Express 5 + MySQL + LanceDB + LangChain + MCP）
│   ├── doc/                    # 知识库源文档（kb:sync 的同步对象，如 knowledge.json）
│   ├── context/                # 提示词模板（context.md，含 ${userFeature} 占位符）
│   ├── tests/                  # Vitest 单元测试（RAG 管线 / 配置中心 / kb 路由等 13 个文件）
│   └── src/
│       ├── bootstrap.js        # 启动引导：配置缺失时输出可执行修复提示而非裸堆栈
│       ├── index.js            # HTTP 入口：/chat(SSE) /new /all /title /singleChat /userFeature /changeImg
│       ├── env.js              # 配置中心：读取 src/.env，启动期校验（必填 / 类型 / 跨字段一致性）
│       ├── mysql.js            # 连接池 + initTables（chat_session / chat_message 自动建表，库名默认 manage_core）
│       ├── embedding.js        # 知识库 CLI 入口（kb:sync / kb:rebuild / kb:stats）
│       ├── store.js            # 向量库基础读写封装（供 CLI 与 RAG 管线复用）
│       ├── rag/                # RAG 管线：rewriter（Query 改写）· chunker（切块）· loader · embedding · indexer（入库）· vectorStore（LanceDB）· retriever（检索）· prompt · manifest · errors（降级分类）
│       ├── MCP/                # MCP 客户端 / 服务端封装（本地 + 远程工具接入）
│       ├── routes/             # health.js（/health 健康检查）· kb.js（/kb/* 知识库增量接口 + 串行队列 + 背压）
│       ├── model/              # 文生图模型调用冒烟脚本
│       ├── utils/              # 工具清单（本地工具 / 前端卡片工具）、用户画像、消息摘要
│       └── .env.example        # 配置模板（复制为同目录 src/.env；src/.env 与 src/data/ 为本地运行时生成物，均不入库）
│
...
统一使用 npm（仓库根目录提供 package-lock.json；若此前用 pnpm 安装过，建议删除子项目内旧的 pnpm-lock.yaml 后统一以 npm 安装，避免两套锁文件混用）：
...
1. AI 后端：复制 `admin-ai-backend/src/.env.example` 为**同目录下**的 `src/.env`，必填：
   ...
   令牌一致性说明：AI 后端未配置 `KB_SYNC_TOKEN` 时，`/kb/*` 处于无鉴权模式（仅限本地开发，启动日志会告警）；一旦配置，业务后端与前端必须携带相同令牌，否则返回 401。
2. 前端：`admin-front/.env.development` 已默认指向 `localhost:3001`（业务）与 `localhost:3000`（AI）；如需启用知识库管理鉴权，在同目录新建 `.env.development.local`（已被 `*.local` 忽略规则排除，不入库）填 `VITE_KB_SYNC_TOKEN`。
...
| 账号     | 密码         | 说明                                     |
| -------- | ------------ | ---------------------------------------- |
| admin    | 123456       | 超级管理员（通配权限 `*`），用于对比权限差异 |
| 李四666  | 123456lisi   | 普通角色账号                             |
| zhangsan | 1234567      | 普通角色账号                             |
| test01 / caiwu / yunwei | 123456 | 普通角色账号                         |

> 完整账号与角色定义见 `admin-mock-backend/src/mock/user/data.ts`。服务重启后新增账号会丢失（内存态预期行为）。建议分别用 admin 与普通账号登录，对比按钮级权限（`v-hasBtn`）与后端 403 的表现。

### 运行测试

```bash
cd admin-front && npm test              # 前端单测（Vitest + happy-dom）
cd admin-front && npm run test:coverage # 前端覆盖率报告
cd admin-ai-backend && npm test         # AI 后端单测（RAG 管线 / 配置中心 / kb 路由等）
cd admin-ai-backend && npm run test:coverage
```
...
- 业务后端**正常响应**（含鉴权失败）的 HTTP 状态码恒为 200，业务码在返回体 `code` 字段（200 成功 / 201 业务失败 / 401 未登录 / 403 无权限），鉴权头 `Authorization: Bearer <token>`；未匹配路由返回 HTTP 404 JSON 兜底，未捕获异常返回 HTTP 500 JSON
- AI 后端除 `/chat`（SSE 流式）外均为 JSON，使用真实 HTTP 状态码（200 / 202 / 401 / 503 等）
- `/kb/*` 接口通过请求头 `x-kb-token`（或 `Authorization: Bearer <token>`）鉴权
- AI 后端接口中的 `convertId` 为会话唯一标识（历史沿用的字段命名，即 conversationId）
...
| 接口路径               | 请求方式 | 参数说明                                                                  |
| ---------------------- | -------- | ------------------------------------------------------------------------- |
| /api/login             | POST     | `{username, password}`，公开接口，返回 token / refreshToken（后者为占位实现，暂不校验） |
| /api/user/login        | POST     | 等价的第二登录入口，返回 token 与用户信息（密码置空）                     |
...
### 审核与日志（审核权限码：goods:review:audit，与商品写权限分离）
...
1. **登录失败？** 当前业务后端为内存态 mock 数据，请使用上文「默认测试账号」表中的内置账号，用户名或密码错误会返回 401；服务重启后新增的账号会丢失，属预期行为。注意：当前版本使用自签 token（非 JWT），登录失败与「JWT 配置」无关；接生产后端时应替换为 JWT 验签或会话存储（代码中已预留替换边界）。
2. **页面报 `net::ERR_CONNECTION_REFUSED` / AI 服务不可用？** 依次检查 AI 后端（3000）与业务后端（3001）是否启动；确认前端 `.env.development` 的 `VITE_AI_API_BASE_URL` 端口与 AI 后端 `src/.env` 的 `PORT` 一致。
...
2. 认证升级：token 替换为 JWT 验签或服务端会话存储（session 模块已整体隔离，可无缝替换；refreshToken 当前为占位下发，刷新闭环随本次升级一并补齐）
3. 实现商品总数统计、商品访问统计、分类统计、报表导出功能，辅助业务数据分析
4. 知识库数据实现标签权限入库：检索阶段先做权限过滤，再将结果返回给用户
5. 工具调用深化：将后台业务操作（商品上下架、审核流转等）注册为模型工具，实现 AI 辅助操作后台（本地工具与前端卡片工具已落地）
